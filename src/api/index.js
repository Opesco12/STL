...
const BASE_URL = "https://stl-proxy.vercel.app/api/v1";
...
export const createClientBank = async (requestData) => {
  try {
    ...
    console.log(error);
    showMessage({
      message: "Invalid details or error while processing request",
      type: "warning",
    });
  }
};
...
export const getclientbankaccounts = async () => {
  try {
    ...
    console.log(error);
    showMessage({
      message: "Unable to fetch client bank accounts",
      type: "warning",
    });
  }
};

export const walletWithdrawalRequest = async (requestData) => {
  try {
    const data = await apiCall({
...
export const checkVerification = async () => {
  try {
    ...
    console.log(error);
    showMessage({ message: "Unable to fetch KYC Status", type: "warning" });
  }
};
